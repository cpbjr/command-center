...
- [ ] **Promote durable brand devices to canonical CSS.** `.meta-rail`, `.section-eyebrow`, `.drawer-grip`, bottom-nav chrome, wordmark lockup currently live in `src/index.css`. Christopher floated moving durable ones into `.agent/SOPs/wpa-brand.css` so client-facing materials share them. **Confirm which before moving** — that file styles external deliverables.
