...
import {
  ...
  type ClientActivity,
  type ClientActivityInsert,
  type ActivityType,
} from '@/hooks/use-client-activity'
import { formatDate } from '@/lib/format'

interface CommLogWidgetProps {
  clientId: number
}
...
export function CommLogWidget({ clientId }: CommLogWidgetProps) {
  const { data: activities = [] } = useClientActivity(clientId)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload: ClientActivityInsert = {
      client_id: clientId,
      type,
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        {recent.map((activity: ClientActivity) => {
